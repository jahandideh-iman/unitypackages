...
- `BasicInventory<T>`, renamed to `Inventory<T>`. It was the only implementation of `IInventory<T>`, so the `Basic` prefix distinguished it from nothing. **Breaking** — update references to the new name.
...
First release of _Inventory System_.
