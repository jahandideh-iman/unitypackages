...
- `IDependencyResolver` and its reflection-based implementation `ReflectionBasedDependencyResolver`, with `RegisterType<T>`, `RegisterFactory<T>` and `RegisterInstance<T>`.
...
- `DependencyResolverExtensions`, typed `RegisterFactory` overloads for factories taking zero to nine dependencies.
